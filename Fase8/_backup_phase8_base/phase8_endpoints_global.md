...
**Estado**: ✅ Activo  
**Propósito**: Permitir que el usuario declare su rol objetivo (por ejemplo, pasar de camarero a arquitecto) para no depender solo de su historial del CV.

| Método | Ruta | Servicio | Descripción |
|--------|------|---------|------------|
| `POST` | `/profiles/cv` | Profile Service | Ahora acepta opcionalmente `desired_role` y `transition_summary` junto al PDF |
| `PUT` | `/profiles/career-goals` | Profile Service | Guarda/actualiza el formulario de cambio profesional del usuario autenticado |
| `GET` | `/profiles/me` | Profile Service | Devuelve el perfil incluyendo `desired_role` y `transition_summary` si existen |
...
curl -X PUT http://localhost/profiles/career-goals \
	...
	-d '{
		...
		"transition_summary": "He trabajado en hosteleria y estoy reorientando mi carrera a arquitectura"
	}'
```

**Notas**:
- `/profiles/career-goals` requiere que el usuario ya tenga CV subido.
- El formulario se guarda en PostgreSQL y tambien se incorpora al JSON ETL de MinIO al subir CV.
- Esta fase no rompe endpoints previos: amplía contrato de perfil con datos de objetivo profesional.
