...
- Captura de objetivo profesional del usuario aunque su CV refleje experiencia previa distinta
- Endpoint dedicado para guardar formulario de cambio de carrera (`PUT /profiles/career-goals`)
- Extension de subida de CV para aceptar datos opcionales del formulario (`desired_role`, `transition_summary`)
- Persistencia de objetivo profesional en PostgreSQL (`profiles.desired_role`, `profiles.transition_summary`)
- Inclusión del objetivo profesional en el JSON ETL guardado en MinIO
- Validacion integral de endpoint nuevo en script global (`scripts/test_all_endpoints.sh`)
...
**FASE A DESARROLLAR: [FASE 8 - Formulario de Transicion Profesional]**

Actúa como mi Arquitecto de Proyectos IT y desarrolla esta fase siguiendo todos los principios anteriores.

Entregables requeridos:
1. Implementar formulario de objetivo profesional en profile-service para cambio de carrera
2. Crear endpoint `PUT /profiles/career-goals` protegido con JWT
3. Extender `POST /profiles/cv` para aceptar opcionalmente `desired_role` y `transition_summary`
4. Persistir datos de formulario en PostgreSQL (`profiles.desired_role`, `profiles.transition_summary`)
5. Incluir datos de formulario en JSON ETL de MinIO
6. Mantener compatibilidad de endpoints de negocio existentes
7. Actualizar `endpoints.md` (global) con rutas funcionales de Fase 8
8. Crear `Fase8/README.md`, `Fase8/ENDPOINTS.md`, `Fase8/criterios-aceptacion.md`, `Fase8/explicacion-archivos.md`
9. Crear backup inmutable en `Fase8/_backup_phase8_base/`
10. Verificacion basica tecnica del endpoint nuevo y del flujo profile
...
## Registro operativo (2026-03-31) - Cumplimiento documental obligatorio Fase 8

Se deja explicitamente registrado que la captura de objetivo profesional para transicion de carrera queda documentada en los archivos obligatorios de fase y globales:

- `Fase8/README.md`
- `Fase8/ENDPOINTS.md`
- `Fase8/criterios-aceptacion.md`
- `Fase8/explicacion-archivos.md`
- `endpoints.md`
- `explicacion-archivos.md`

Incluye:
- Endpoint `PUT /profiles/career-goals` con JWT para guardar formulario de cambio profesional.
- Extension de `POST /profiles/cv` para aceptar `desired_role` y `transition_summary` en multipart.
- Persistencia de `desired_role` y `transition_summary` en tabla `profiles`.
- Inclusion de `career_goal` en JSON ETL de MinIO.
- Validacion integral de endpoint nuevo dentro de `scripts/test_all_endpoints.sh`.
- Backup inmutable de fase para trazabilidad y recuperacion.