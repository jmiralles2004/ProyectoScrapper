...
PHASE7_ENV_FILE="$RESOLVED_ENV_FILE" "$PYTHON_BIN" - <<'PY'
import json
import os
import time
import uuid
...
env_values = load_env_file(os.environ["PHASE7_ENV_FILE"])
...
ensure_status(client.get(f"{base_url}/profiles/me", headers=auth_headers), 200, "Profile me")
...
summary_path = Path("/tmp/phase7-endpoints-e2e-summary.json")
